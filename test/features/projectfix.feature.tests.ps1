...
Describe "fix a project with missing references" {
    ...
    It "should detect missing references" {
        ...
        $missing[0].ref.Name | Should -Be "Core.Library1"
    }
    It "should fix references paths" {
        fixcsproj $csproj -reporoot "$targetdir/test" -Verbose
    
        $deps = get-csprojdependencies $csproj
        $fixed = $deps | ? { $_.ref.Name -eq "Core.Library1" }
        
        $fixed.Path | Should -Be "..\..\Core2\Core.Library1\Core.Library1.csproj"
        $missing = @($deps | ? { $_.ref.isvalid -eq $false })
