...
function GetInternal-SharedMSDeployParametersFrom{
    ...
    process{
        ...
        if ($publishProperties['_DestinationType'] -ne 'AzureWebSite') { 
            # allow untrusted publish
...
function Publish-AspNet{
    ...
    process{
		write-output "Publish properties:"
		$publishProperties | out-string | write-output
...
function Publish-AspNetMSDeploy{
    ...
    process{
        if($publishProperties){
            ...
            if ($publishProperties['MSDeployServiceURL'] -eq $null) {
				throw "missing property 'MSDeployServiceURL' in publish profile!"
            }
            $subdir = ""
            if ($publishProperties['wwwRootOnly'] -ne $null -and $publishProperties['wwwRootOnly'] -ieq "true") {
                $subdir = "/wwwroot"
            }
            if ($publishProperties['subdir'] -ne $null -and $publishProperties['subdir'] -ne "") {
                $subdir = "/" + $publishProperties['subdir'].Trim("/")
            }

            $authType = "Basic"
            if ($publishProperties['AuthType'] -ne $null) {
                $authType = $publishProperties['AuthType'] 
            }

            $publishArgs = @()
            $publishArgs += ('-source:IisApp=''{0}''' -f "$webrootOutputFolder$subdir")
            $publishArgs += ('-dest:IisApp=''{0}'',ComputerName=''{1}'',UserName=''{2}'',Password=''{3}'',IncludeAcls=''False'',AuthType=''{5}''{4}' -f 
                                    "$($publishProperties['DeployIisAppPath'])$subdir",
            ...
            if ($publishProperties['_DestinationType'] -eq 'AzureWebSite') { 
            ...
            if ([string]::IsNullOrEmpty($publishPwd)) { 
                Write-Warning "Empty password given!"
                $publishPwd = ""
...
function Recycle-AspNetMSDeploy{
    [cmdletbinding(SupportsShouldProcess=$true)]
    param(
        [Parameter(Mandatory = $true,Position=0,ValueFromPipeline=$true,ValueFromPipelineByPropertyName=$true)]
        $publishProperties,
    ...
    process{
        if($publishProperties){
            ...
            "C:\Program Files (x86)\IIS\Microsoft Web Deploy V3\msdeploy.exe" 
                -source:recycleApp 
                -dest:recycleApp="IIS_SITE_NAME",recycleMode="StopAppPool",ComputerName="https://CLOUD_SERVICE_NAME.cloudapp.net:PORT/msdeploy.axd",UserName="USERNAME",Password="PASSWORD",AuthType="Basic"
                -verb:sync 
                -allowUntrusted 
            #>
			...
			write-verbose "doing recycle:$recycleMode app $( $publishProperties['DeployIisAppPath']) at machine $((Get-MSDeployFullUrlFor -msdeployServiceUrl $publishProperties['MSDeployServiceURL']))"
            ...
            $publishArgs = @()
            $publishArgs += ('-source:recycleApp')
            $publishArgs += ('-dest:recycleApp=''{0}'',recycleMode=''{5}'',ComputerName=''{1}'',UserName=''{2}'',Password=''{3}'',IncludeAcls=''False'',AuthType=''{6}''{4}' -f 
...
function Get-MSDeployFullUrlFor{
    ...
    process{
        ...
        if ($msdeployServiceUrl -eq $null) {
			throw "msdeployServiceUrl is NULL"
        }
        if ($msdeployserviceurl.StartsWith("http")) {
			return $msdeployserviceurl
        }
        return 'https://{0}/msdeploy.axd' -f $msdeployServiceUrl.TrimEnd(':443')
    }
}
...
function InternalRegister-AspNetKnownPublishHandlers{
    ...
    process{
        ...
        Register-AspnetPublishHandler -name 'MSDeploy' -force -handler {
            ...
            $recycled = $false
            if ($publishProperties['_DestinationType'] -ne 'AzureWebSite' -and $publishProperties['Recycle'] -ine "false") { 
                try {                        
                    $recycled = $true
                    Recycle-AspNetMSDeploy -publishProperties $publishProperties -packOutput $packOutput -recycleMode "StopAppPool"
            ...
            if ($publishProperties['_DestinationType'] -ne 'AzureWebSite' -and $recycled) {            
                try {
                    Recycle-AspNetMSDeploy -publishProperties $publishProperties -packOutput $packOutput -recycleMode "StartAppPool"
                } catch {
                    Write-Warning $_
                }
            }
